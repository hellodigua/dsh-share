window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function cloneMessage(source, locale, hideReasoning = false) {
    ...
    <span style="background:currentColor;border-radius:2px;color:var(--dsw-alias-label-primary-inverted,#fff);font-size:8px;font-weight:700;letter-spacing:.08em;padding:1px 4px">HARNESS</span>`;
		...
		function createShareCard(document, content, locale, settings = DEFAULT_SHARE_SETTINGS) {
			...
			const answers = settings.hideProcess ? content.answers.filter((answer) => answer.dataset.chatFlowKind === "assistant-step").slice(-1) : content.answers;
			for (const answer of answers) answerSection.append(cloneMessage(answer, locale, settings.hideProcess));
...
`;
      ...
      <div class="dsh-share-dialog__footer">
		...
		function ShareAction({ shareRuntime }) {
